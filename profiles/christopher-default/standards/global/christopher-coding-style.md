...
- **.env practices**: For local development, store environment variables in a `.env` file and load them with `dotenv`. Do not commit `.env` to version control. Always keep a `sample.env` file up to date to document all possible environment variables. For Docker, pass environment variables at runtime (not build time); don’t rely on `.env` files inside containers.